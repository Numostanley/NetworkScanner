...
class WapitiScannerTest(TestCase):
    ...
    def test_host_key_in_query_params(self):
        response = self.client.get(f'{reverse("wapiti:scan")}?')
        self.assertEqual(response.status_code, 400)

    def test_host_key_value_not_specified_in_query_params(self):
        response = self.client.get(f'{reverse("wapiti:scan")}?host=')
        self.assertEqual(response.status_code, 400)

    def test_wapiti_scan_is_in_progress(self):
        response = self.client.get(f'{reverse("wapiti:scan")}?host={self.host}')
...
class WapitiScanResultTest(TestCase):
    ...
    def test_host_key_in_query_params(self):
        ...
        response = self.client.get(f'{reverse("wapiti:result")}?')
        self.assertEqual(response.status_code, 400)

    def test_host_key_value_not_specified_in_query_params(self):
        response = self.client.get(f'{reverse("wapiti:result")}?host=')
        self.assertEqual(response.status_code, 400)

    def test_host_not_found(self):
        # test if the host is not found
        self.assertIsNone(self.not_found_host)
        response = self.client.get(f'{reverse("wapiti:result")}?host={self.not_found_host}')
        self.assertEqual(response.status_code, 404)

    def test_wapiti_scan_result_does_not_exist_for_host(self):
        response = self.client.get(f'{reverse("wapiti:result")}?host={self.found_host_with_no_result}')
        self.assertEqual(response.status_code, 404)

    def test_wapiti_scan_result_exist_for_host(self):
        response = self.client.get(f'{reverse("wapiti:result")}?host={self.found_host_with_result}')
        self.assertEqual(response.status_code, 200)
