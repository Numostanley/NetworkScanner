...
class WhatWebScannerTest(TestCase):
    ...
    def test_whatweb_scan_successful(self):
        response = self.client.get(f'{BASE_URL}/whatweb/scan?host={self.host}')
...
class WhatWebScanResultTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.found_ip_with_result = Host.get_host('193.122.75.144')
        self.found_ip_with_no_result_scan = Host.get_host('193.122.66.53')
        self.not_found_host = Host.get_host('122.121.33.45')

        self.create_whatweb_scan = WhatWeb.create_whatweb_scan(self.found_ip_with_result,
                                                               whatweb_data[0]['fields']['data']['data'])

        self.get_whatweb_scan_with_result = WhatWeb.get_whatweb_scan_by_host(self.found_ip_with_result)
        self.get_whatweb_scan_with_no_result = WhatWeb.get_whatweb_scan_by_host(self.found_ip_with_no_result_scan)
    ...
    def test_host_not_found(self):
        ...
        response = self.client.get(
            f'{BASE_URL}/whatweb/get-result?host={self.not_found_host}'
        )
        self.assertEqual(response.status_code, 404)

    def test_whatweb_scan_result_does_not_exist_for_host(self):
        response = self.client.get(
            f'{BASE_URL}/whatweb/get-result?host={self.found_ip_with_no_result_scan}'
        )
        if self.get_whatweb_scan_with_no_result.count() < 1:
            self.assertEqual(response.status_code, 404)

    def test_whatweb_scan_result(self):
        response = self.client.get(
            f'{BASE_URL}/whatweb/get-result?host={self.found_ip_with_result}'
        )
        if self.get_whatweb_scan_with_result.count() > 0:
            self.assertEqual(response.status_code, 200)
